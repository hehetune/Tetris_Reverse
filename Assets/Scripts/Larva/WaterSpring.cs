    ...
    public class WaterSpring : MonoBehaviour
    {
        ...
        public void WavePointUpdate()
        {
            if (spriteShapeController != null)
            {
                ...
                waterSpline.SetPosition(waveIndex,
                    new Vector3(wavePosition.x, transform.localPosition.y, wavePosition.z));
            }
        }

        private CharacterMovement _cachePlayer;

        private void OnCollisionEnter2D(Collision2D other)
        {
            if (other.gameObject.tag.Equals("Player"))
            {
                _cachePlayer = other.gameObject.GetComponent<CharacterMovement>();
                // _cachePlayer.SetCanUpdateGravityScale(false);
                var speed = _cachePlayer.Rb.velocity;
        ...
        //         if(!_cachePlayer)
        //             _cachePlayer = other.gameObject.GetComponent<CharacterMovement>();
        //         _cachePlayer.SetCanUpdateGravityScale(true);
        //     }
        // }
    }
}