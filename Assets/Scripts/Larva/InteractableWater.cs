...
namespace Larva
{
    ...
    public class InteractableWater : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            for (int i = 1; i < _waterPoints.Count - 1; i++)
            {
                ...
                float accleration = -_spriteConstant * x - _damping * point.velocity;
                point.pos += point.velocity * _speedMult * Time.fixedDeltaTime;
                _vertices[_topVerticesIndex[i]].y = point.pos;
                point.velocity += accleration * _speedMult * Time.fixedDeltaTime;
            }
            ...
            for (int j = 0; j < _wavePropogationIterations; j++)
            {
                for (int i = 1; i < _waterPoints.Count - 1; i++)
                {
                    float leftDelta = _spread * (_waterPoints[i].pos - _waterPoints[i - 1].pos) * _speedMult *
                                      Time.fixedDeltaTime;
                    _waterPoints[i - 1].velocity += leftDelta;
                    float rightDelta = _spread * (_waterPoints[i].pos - _waterPoints[i + 1].pos) * _speedMult *
                                       Time.fixedDeltaTime;
    ...
    public class InteractableWaterEditor : Editor
    {
        ...
        private void OnSceneGUI()
        {
            ...
            {
                ChangeDimensions(ref _water.Width, ref _water.Height, corners[1].x - newBottomLeft.x,
                    corners[3].y - newBottomLeft.y);
                _water.transform.position = new Vector3(newBottomLeft.x + (corners[1].x - newBottomLeft.x) / 2,
                    newBottomLeft.y + (corners[3].y - newBottomLeft.y) / 2, 0);
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ChangeDimensions(ref _water.Width, ref _water.Height, newBottomRight.x - corners[0].x,
                    corners[3].y - newBottomRight.y);
                _water.transform.position = new Vector3(corners[0].x + (newBottomRight.x - corners[0].x) / 2,
                    newBottomRight.y + (corners[3].y - newBottomRight.y) / 2, 0);
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ChangeDimensions(ref _water.Width, ref _water.Height, corners[3].x - newTopLeft.x,
                    newTopLeft.y - corners[0].y);
                _water.transform.position = new Vector3(newTopLeft.x + (corners[3].x - newTopLeft.x) / 2,
                    corners[0].y + (newTopLeft.y - corners[0].y) / 2, 0);
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ChangeDimensions(ref _water.Width, ref _water.Height, newTopRight.x - corners[2].x,
                    newTopRight.y - corners[1].y);
                _water.transform.position = new Vector3(corners[2].x + (newTopRight.x - corners[2].x) / 2,
                    newTopRight.y + (newTopRight.y - corners[1].y) / 2, 0);
            }
